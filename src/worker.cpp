...
void enqueue_task(const Task& t) {
    ...
    q_not_full.wait(lk, []{ return q_count < QMAX; });
    queue_buf[q_tail] = t;
...
Task dequeue_task() {
    unique_lock<mutex> lk(q_lock);
    q_not_empty.wait(lk, []{ return q_count > 0; });
...
void apply_slice_block(int rs, int re, int t1, int t2) {
    for (int r = rs; r < re; ++r) {
        ...
        for (int c = 0; c < g_in.w; ++c) {
            ...
            if (z <= t1 || z >= t2) g_out.data[base + c] = 255;
            else g_out.data[base + c] = z;
        }
    }
}
...
bool read_from_fifo(const string& fifo) {
    ...
    if (r != (ssize_t)sizeof(hdr)) { close(fd); return false; }
    ...
    while (total < payload) {
        ...
        if (n <= 0) { perror("read data"); close(fd); return false; }
        total += n;
    }
    close(fd);
    return true;
...
int main(int argc, char** argv) {
    ...
    if (mode == "negativo") {
        g_mode = MODE_NEG;
        if (argc >= 5) g_nthreads = stoi(argv[4]);
    } else if (mode == "slice") {
        g_mode = MODE_SLICE;
        if (argc < 6) {
        ...
        g_t1 = stoi(argv[4]);
        g_t2 = stoi(argv[5]);
        if (argc >= 7) g_nthreads = stoi(argv[6]);
    } else {
        cerr << "Modo inválido\n";
        return 1;
    }
